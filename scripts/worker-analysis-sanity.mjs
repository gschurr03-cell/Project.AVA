...
try {
  ...
  const { toAnalysisMetrics, CALIBRATION_WARNING } = require(path.join(out, "biomechanics/worker/index.js"));
  const { analysisMetricsSchema } = require(path.join(out, "biomechanics/types.js"));
  ...
  const partial = toAnalysisMetrics(
    result({ strideFrequencyHz: 2.5, avgGroundContactMs: 120, avgTrunkLeanDeg: 5.2 }),
  );
  check(`partial → schema-valid AnalysisMetrics`, valid(partial.metrics));
  check(
    `MVP scope: step frequency preserved (${partial.metrics.strideFrequencyHz} Hz), forbidden timing/angle metrics withheld (gc=${partial.metrics.groundContactTimeMs}, trunk=${partial.metrics.avgTrunkLeanDeg})`,
    partial.metrics.strideFrequencyHz === 2.5 &&
      partial.metrics.groundContactTimeMs === null &&
      partial.metrics.avgTrunkLeanDeg === null,
  );
  check(
    `partial → uncomputed fields remain unavailable (null)`,
    partial.metrics.flightTimeMs === null &&
      partial.metrics.topSpeedMps === null &&
      partial.metrics.avgStrideLengthM === null,
  );

  // (2) Warnings preserved (analysis warnings + calibration warning).
  const warned = toAnalysisMetrics(
    result({}, ["Metrics are partial; some values could not be computed."]),
  );
  check(
    `warnings preserved (${warned.warnings.length}: includes analysis + calibration)`,
    warned.warnings.includes("Metrics are partial; some values could not be computed.") &&
      warned.warnings.includes(CALIBRATION_WARNING),
  );
  ...
  let empty;
  try {
    empty = toAnalysisMetrics(result({}));
  } catch {
    threw = true;
  }
  check(
    `empty result → no throw, unavailable metrics stay null`,
    !threw && valid(empty.metrics) && empty.metrics.strideFrequencyHz === null,
  );

  // (4) MVP scope: knee-flexion (and other advanced angle diagnostics) are no longer
  //     computed or serialized by the pipeline — they are withheld as null.
  const bothKnees = toAnalysisMetrics(
    result({ peakLeftKneeFlexionDeg: 30, peakRightKneeFlexionDeg: 20 }),
  );
  check(
    `peakKneeFlexionDeg withheld (null) under MVP scope even when interior angles are present`,
    bothKnees.metrics.peakKneeFlexionDeg === null,
  );
