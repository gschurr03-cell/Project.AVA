// Phase 4.1 (2026-08-05) — fix for a bug in scripts/phase-4-1-real-reruns.mjs:
// that script reused `current.input_snapshot` verbatim (mirroring the
// panning-fixture "--requeue" test pattern), but for vanni_fly_120 and
// vanni_fly_60 that column was ALREADY null in the database before this
// script ever touched it (a pre-existing gap, not caused by the box_tracker
// change) — so the requeued jobs failed downstream at "uploading_artifacts"
// with a Zod "expected object, received null" error the moment the real
// pipeline actually needed to build a result from it.
//
// The real "rerun analysis" Server Action (queueAnalysis in
// src/app/sessions/actions.ts) never reuses the old input_snapshot at all —
// it always builds a FRESH one from the session's own live, current
// database fields (this is standard behavior, not a special case). This
// script replicates that exact construction (same field list, same shape)
// for the two affected sessions only, using real, live, non-fabricated
// session/athlete data, and requeues them the same way.
...
const TARGETS = [
  { label: "vanni_fly_120", sessionId: "160a86a2-c0db-4e7d-9fbe-82aedd6d3eff", analysisId: "6d9a6aba-d099-4a33-b8ea-2dd4962fe80c" },
  { label: "vanni_fly_60", sessionId: "3d6ba4b6-a3b4-450a-b9f0-ef36a1311b8d", analysisId: "8f55936c-cf07-4c20-ba73-b662e8d24325" },
];

const results = [];
for (const { label, sessionId, analysisId } of TARGETS) {
  ...
  const { data: current, error: currentError } = await db
    ...
    .eq("id", analysisId)
    .single();
  if (currentError || !current) throw currentError ?? new Error(`${label}: current analysis missing`);

  // Exactly mirrors queueAnalysis's requestedAnalysisFps derivation
  // (src/app/sessions/actions.ts) for a previously-analyzed native/validated
  // source: round the session's own real detected fps to 3 decimals.
  ...
  if (requeuedId !== analysisId) {
    throw new Error(`${label}: expected to reuse ${analysisId}, got ${requeuedId}`);
  }
  results.push({ label, sessionId, workingAnalysisId: requeuedId, snapshotBuiltFresh: true });
}
