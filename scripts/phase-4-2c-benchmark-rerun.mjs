// Phase 4.2C (2026-08-06) — general-purpose real production rerun for any
// one of the 4 registered benchmarks, through the real worker/queue path
// (`replace_working_analysis`, the same RPC `queueAnalysis` uses), with the
// pre-run working analysis saved as an immutable version first
// (`save_working_analysis_snapshot`) — mirrors Phase 1/4.1/4.2B's own
// established pattern. Usage: node scripts/phase-4-2c-benchmark-rerun.mjs <key>
// where <key> is one of: gav, vanni240, vanni120, vanni60.
...
const BENCHMARKS = {
  gav: { label: "gav_stationary_reference", sessionId: "e04a7983-7406-4a00-bb89-8ada7b10bf9f", analysisId: "3a148f45-02ff-492d-b9f1-790470b83c21" },
  vanni240: { label: "vanni_fly_240", sessionId: "31fe352b-f00f-4a80-b20a-17c2ab08ec5a", analysisId: "a7679326-e193-4489-bf50-735fe402ec60" },
  vanni120: { label: "vanni_fly_120", sessionId: "160a86a2-c0db-4e7d-9fbe-82aedd6d3eff", analysisId: "6d9a6aba-d099-4a33-b8ea-2dd4962fe80c" },
  vanni60: { label: "vanni_fly_60", sessionId: "3d6ba4b6-a3b4-450a-b9f0-ef36a1311b8d", analysisId: "8f55936c-cf07-4c20-ba73-b662e8d24325" },
};

const target = BENCHMARKS[process.argv[2]];
if (!target) {
  throw new Error(`Usage: node scripts/phase-4-2c-benchmark-rerun.mjs <${Object.keys(BENCHMARKS).join("|")}>`);
}
const { label, sessionId, analysisId } = target;
...
const { data: savedVersionId, error: saveError } = await db.rpc("save_working_analysis_snapshot", {
  ...
  p_notes: `Phase 4.2C pre-rerun snapshot (${label}, crop-handoff provenance + pose feedback + detector-cost changes, 2026-08-06)`,
});
if (saveError || !savedVersionId) throw saveError ?? new Error(`${label}: save_working_analysis_snapshot failed`);
...
console.log(JSON.stringify({ label, sessionId, analysisId, savedVersionId, requeuedId }, null, 2));
