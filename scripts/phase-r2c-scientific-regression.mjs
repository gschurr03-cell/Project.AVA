// Phase R2C Part K -- scientific regression: confirm zone time, crossings,
// step frequency, step length, velocity, contacts, and steps are byte-
// identical to the last known-good (Phase R2) production snapshot, since
...
//
//   node scripts/phase-r2c-scientific-regression.mjs
...
const out = path.join(root, ".r2c-sci-tmp");
...
try {
  ...
  // Compare against the R2-recorded baseline (last known-good, immediately prior phase).
  let allMatch = true;
  const priorBaseline = JSON.parse(readFileSync(path.join(root, "tmp/phaseR2/scientific-before-after.json"), "utf8")).current;
  const comparison = {};
  for (const label of Object.keys(BENCHMARKS)) {
    const prior = priorBaseline[label];
    const cur = results[label];
    const match = prior && prior.totalContacts === cur.totalContacts && prior.zoneTimeS === cur.zoneTimeS
      && prior.avgIndividualStepLengthM === cur.avgIndividualStepLengthM
  ...
  console.log(`\nAll benchmarks match R2 baseline: ${allMatch}`);
  writeFileSync(path.join(root, "tmp/phaseR2C/scientific-before-after.json"), JSON.stringify({ current: results, comparisonToR2Baseline: comparison, allMatch }, null, 2));
} finally {
