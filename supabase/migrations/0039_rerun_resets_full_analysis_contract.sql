-- Fix: a rerun of a previously-EXPERIMENTAL working analysis violated
-- `analyses_experimental_contract_valid`.
--
-- `replace_working_analysis` (0035) reset experimental/experiment_version/
-- validation_status back to the validated contract on rerun, but left
-- `compatibility_group` at its prior value. When the current working row had been
-- completed as an experimental-30 analysis (compatibility_group='experimental-30-v1'
-- via `complete_experimental_analysis_job`), the rerun produced the invalid mix
--   (not experimental) AND (compatibility_group <> 'validated-60-v1')
-- which satisfies neither arm of the CHECK.
--
-- The constraint is CORRECT and unchanged. This redefines the function so a rerun
-- resets the FULL validated contract (adding compatibility_group='validated-60-v1'),
-- and makes the new-analysis INSERT set the validated contract explicitly instead of
-- relying on column defaults. Mirrors src/lib/analysis/analysisContract.ts.
...
begin
  ...
  if v_id is null then
    insert into public.analyses (
      ...
      -- Validated analysis contract (explicit, never relying on defaults).
      experimental, experiment_version, validation_status, compatibility_group
    ) values (
      ...
      false, null, 'validated', 'validated-60-v1'
    ...
    update public.analyses set
      ...
      -- Reset the FULL validated analysis contract. compatibility_group MUST return to
      -- 'validated-60-v1' here — omitting it left a stale experimental group and broke
      -- analyses_experimental_contract_valid on rerun of an experimental working row.
      experimental=false, experiment_version=null, experimental_result=null,
      validation_status='validated', compatibility_group='validated-60-v1',
      raw_timing_metrics=null, reported_timing_metrics=null,
