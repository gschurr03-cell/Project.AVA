...
const serverSchema = publicSchema.extend({
  ...
  ANALYSIS_SUBMISSION_ENABLED: z.enum(["true","false"]).default("true"),
  ...
  if(["staging","closed_beta","production"].includes(value.AVA_ENVIRONMENT)){
    ...
    if(value.NEXT_PUBLIC_APP_URL.includes("localhost")||value.NEXT_PUBLIC_APP_URL.includes("127.0.0.1"))
      context.addIssue({code:"custom",path:["NEXT_PUBLIC_APP_URL"],message:"A public application URL is required."});
...
export function serverEnv() {
  return serverSchema.parse({
    ...
    ANALYSIS_SUBMISSION_ENABLED: process.env.ANALYSIS_SUBMISSION_ENABLED,
