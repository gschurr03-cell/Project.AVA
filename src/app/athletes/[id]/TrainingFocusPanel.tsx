...
const TREND_STYLE: Record<FocusTrend, { label: string; className: string }> = {
  worsening: { label: "Worsening", className: "border border-[#e46464]/40 bg-[#e46464]/12 text-[#e46464]" },
  improving: { label: "Improving", className: "border border-[#f5c451]/40 bg-[#f5c451]/12 text-[#f5c451]" },
  steady: { label: "Steady", className: "border border-white/10 bg-white/[0.05] text-[#b3bccb]" },
};
...
export default function TrainingFocusPanel({ focus }: { focus: TrainingFocus }) {
  ...
  if (focus.sessionsAnalyzed === 0) {
    return (
      <section className="rounded-2xl border border-white/[0.06] bg-[#101827]/95 p-5">
    ...
    return (
      <section className="rounded-2xl border border-white/[0.06] bg-[#101827]/95 p-5">
        ...
        <p className="mt-2 text-sm text-[#b3bccb]">
          <span className="mr-1 text-[#f5c451]">✓</span>
  ...
  return (
    <section className="space-y-6 rounded-2xl border border-white/[0.06] bg-[#101827]/95 p-5">
      ...
      <div className="rounded-xl border border-white/[0.06] bg-[#182233] p-4">
        ...
        <h3 className="mt-2 text-lg font-semibold text-[#f5f7fb]">
          {primary.title}
          <span className="ml-2 align-middle text-xs font-medium uppercase tracking-wide text-[#7e8797]">
            {primary.category}
          </span>
        </h3>

        <p className="mt-1 text-xs text-[#7e8797]">{persistenceLabel(primary)}</p>

        <p className={`mt-3 ${LABEL}`}>What to work on</p>
        <p className="text-sm text-[#b3bccb]">{primary.explanation}</p>

        <p className={`mt-3 ${LABEL}`}>Why this matters</p>
        <p className="text-sm text-[#b3bccb]">{primary.rationale}</p>
      ...
      {secondary.length > 0 && (
        <div>
          <h3 className="mb-2 text-base font-semibold text-[#f5f7fb]">Secondary Focus Areas</h3>
          <div className="space-y-3">
            {secondary.map((area) => (
              <div key={area.id} className="rounded-xl border border-white/[0.06] bg-[#182233] p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-semibold text-[#f5f7fb]">{area.title}</p>
                    <p className="mt-0.5 text-xs text-[#7e8797]">{persistenceLabel(area)}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <TrendBadge trend={area.trend} />
                    <span className="text-xs font-medium text-[#7e8797]">{area.focusScore}</span>
                  </div>
                </div>

                <p className="mt-2 text-sm text-[#b3bccb]">{area.explanation}</p>
